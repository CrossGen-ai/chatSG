...
exports.OrchestratorFactory = exports.AgentOrchestrator = void 0;
...
class AgentOrchestrator {
     ...
     */
    async selectAgent(input, context) {
        ...
        try {
            ...
            return {
                ...
                confidence: 0.1,
                reason: `Fallback selection due to error: ${error.message}`,
    ...
    async delegateTask(task, targetAgent) {
        ...
        try {
            ...
            const result = await this.executeWithTimeout(() => agent.processMessage(task.input, context.sessionId), this.delegationConfig.timeoutMs);
            ...
            if (this.delegationConfig.fallbackStrategy !== 'best-effort') {
                return this.handleTaskFailure(task, targetAgent, error);
            }
            return {
                taskId: task.id,
                success: false,
     ...
     */
    async handleConversationHandoff(fromAgent, toAgent, context) {
        try {
            ...
            const stateContext = (0, utils_1.createStateContext)(context.sessionId, fromAgent);
            // Store handoff information in shared state
            await this.stateManager.setSharedState(`handoff:${context.sessionId}:${Date.now()}`, {
                fromAgent,
                toAgent,
            ...
            await this.stateManager.updateSessionState(context.sessionId, {
                data: {
                    activeAgent: toAgent,
    ...
    initializeStrategies() {
        ...
        this.strategies.set('simple', {
            name: 'simple',
            async selectAgent(context, agents) {
        ...
        this.strategies.set('capability-based', {
            ...
            async shouldHandoff(currentAgent, context, agents) {
                // Check if there's a significantly better agent for the current input
                const currentAgentScore = self.calculateCapabilityScore(context.userInput, agents.get(currentAgent)?.getCapabilities());
     ...
     */
    calculateCapabilityScore(input, capabilities) {
        if (!capabilities) {
     ...
     */
    calculateKeywordScore(input, capabilities) {
        let score = 0;
        const reasons = [];
     ...
     */
    routeToSpecializedAgent(input, agents) {
        ...
        const analyticalScore = this.countPatternMatches(input, analyticalPatterns);
        const creativeScore = this.countPatternMatches(input, creativePatterns);
        const technicalScore = this.countPatternMatches(input, technicalPatterns);
        ...
        if (topScore.score >= minThreshold && agents.has(topScore.type)) {
            ...
            const matchedKeywords = topScore.patterns.filter(pattern => input.includes(pattern));
            return {
     ...
     */
    countPatternMatches(input, patterns) {
     ...
     */
    getFallbackAgent() {
        // Prefer AgentZero as fallback, then first available
        if (this.agents.has('AgentZero')) {
            return 'AgentZero';
        }
     ...
     */
    async shareTaskContext(task, context) {
        const stateContext = (0, utils_1.createStateContext)(context.sessionId, context.currentAgent);
        await this.stateManager.setSharedState(`task:${task.id}`, {
            task,
            context,
            timestamp: new Date()
        }, {
     ...
     */
    async handleTaskFailure(task, failedAgent, error) {
     ...
     */
    async trySequentialFallback(task, agents, originalError) {
     ...
     */
    async tryParallelFallback(task, agents, originalError) {
        const fallbackPromises = agents.map(agentName => this.delegateTask(task, agentName).catch(error => ({ error, agent: agentName })));
        try {
            ...
            return {
                taskId: task.id,
                success: false,
                error: `Parallel fallback failed: ${error.message}. Original error: ${originalError.message}`,
                executedBy: 'parallel-fallback',
                executionTime: 0
            };
        }
     ...
     */
    generateHandoffMessage(fromAgent, toAgent, context) {
        const templates = [
     ...
     */
    updateAverageSelectionTime(newTime) {
 ...
 */
function createDefaultOrchestrator(config) {
...
class OrchestratorFactory {
    static createDevelopmentOrchestrator() {
    ...
    static createLightweightOrchestrator() {
        return new AgentOrchestrator({
            maxRetries: 1,
            timeoutMs: 10000,
...
exports.OrchestratorFactory = OrchestratorFactory;
