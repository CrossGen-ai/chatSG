...
const MemoryPersistence_1 = require("./persistence/MemoryPersistence");
class StateManager {
    constructor(config) {
        // Core storage
        ...
        this.persistence = new MemoryPersistence_1.MemoryPersistence(this.config.cleanupInterval);
    ...
    // Session Management
    async getSessionState(sessionId, context) {
    ...
    async updateSessionState(sessionId, updates, context) {
        const startTime = Date.now();
        try {
            let sessionState = this.sessions.get(sessionId);
            if (!sessionState) {
    ...
    async deleteSessionState(sessionId, context) {
        const startTime = Date.now();
        try {
    ...
    async getSharedState(key, context) {
        const startTime = Date.now();
        try {
    ...
    async setSharedState(key, data, options, context) {
        const startTime = Date.now();
        try {
    ...
    async deleteSharedState(key, context) {
        const startTime = Date.now();
        try {
            const sharedState = this.sharedStates.get(key);
            if (!sharedState) {
    ...
    // Query Operations
    async queryStates(query, context) {
        const startTime = Date.now();
        try {
            let results = [];
            for (const sharedState of this.sharedStates.values()) {
                ...
                // Apply filters
                if (query.scope && sharedState.scope !== query.scope)
                    continue;
                if (query.prefix && !sharedState.key.startsWith(query.prefix))
                    continue;
                if (query.agentName && sharedState.metadata.createdBy !== query.agentName)
                    continue;
                if (query.userId && sharedState.scope === 'user' && !sharedState.key.includes(query.userId))
                    continue;
                if (query.sessionId && sharedState.scope === 'session' && !sharedState.key.includes(query.sessionId))
                    continue;
                if (query.createdAfter && sharedState.metadata.createdAt < query.createdAfter)
                    continue;
                if (query.createdBefore && sharedState.metadata.createdAt > query.createdBefore)
                    continue;
    ...
    async wrapExistingSession(sessionId, chatHistory) {
    ...
    checkPermission(sharedState, operation, context) {
 ...
 */
function getStateManager(config) {
