...
async function testStateManagement() {
    ...
    try {
        ...
        const updateResult = await stateManager.updateSessionState(sessionId, { data: { userPreferences: { theme: 'dark', language: 'en' }, lastActivity: new Date() } }, context);
        ...
        const globalStateResult = await stateManager.setSharedState('global-config', { apiVersion: '1.0', features: ['chat', 'tools', 'state'] }, {
            scope: 'global',
        ...
        const userStateResult = await stateManager.setSharedState('user-settings', { notifications: true, autoSave: true }, {
            scope: 'user',
        ...
        const sharedDataResult = await stateManager.setSharedState('agent-collaboration', { task: 'process-document', status: 'in-progress', assignedTo: 'Agent2' }, {
            scope: 'global',
        ...
        const updateSharedResult = await stateManager.setSharedState('agent-collaboration', { task: 'process-document', status: 'completed', completedBy: 'Agent2' }, {
            scope: 'global',
            permissions: { read: ['Agent1', 'Agent2'], write: ['Agent1', 'Agent2'], delete: ['Agent1'] }
        }, agent2Context);
        ...
        const restrictedStateResult = await stateManager.setSharedState('restricted-data', { secret: 'classified-information' }, {
            scope: 'global',
        ...
        const shortTTLResult = await stateManager.setSharedState('short-lived', { expires: 'soon' }, {
            scope: 'global',
...
//# sourceMappingURL=test-state-management.js.map