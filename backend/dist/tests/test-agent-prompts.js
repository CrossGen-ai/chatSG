...
async function testAgentPrompts() {
    ...
    try {
        ...
        runTest('Agent Configuration Loading', () => {
            ...
            if (!config.agentInfo)
                throw new Error('Missing agentInfo section');
            if (!config.prompts)
                throw new Error('Missing prompts section');
            if (!config.prompts.system)
                throw new Error('Missing system prompts');
            if (!config.templateVariables)
                throw new Error('Missing templateVariables section');
            return true;
        });
        ...
        runTest('Property-Based Prompt Selection', () => {
            ...
            if (!analyticalPrompt)
                throw new Error('Analytical prompt not found');
            if (!creativePrompt)
                throw new Error('Creative prompt not found');
            if (!technicalPrompt)
                throw new Error('Technical prompt not found');
        ...
        runTest('Direct Prompt Access', () => {
            const sessionContext = llmHelper.getAgentPrompt('AgentZero', 'customInstructions.sessionContext', { sessionId: 'test_session_123' });
            const userTemplate = llmHelper.getAgentPrompt('AgentZero', 'userTemplates.analytical', { userInput: 'test input' });
            if (!sessionContext)
        ...
        runTest('Template Variable Substitution', () => {
            ...
            testCases.forEach((testCase, index) => {
                const result = llmHelper.substituteTemplateVariables(testCase.template, testCase.context);
        ...
        runTest('Missing Variable Handling', () => {
            const result = llmHelper.substituteTemplateVariables('Hello {userName}, missing: {missingVar}', { userName: 'Alice' });
            ...
            const result2 = llmHelper.substituteTemplateVariables('Hello {userName}, empty: {emptyVar}', { userName: 'Alice', emptyVar: '' });
        ...
        runTest('Context-Aware Prompt Generation', () => {
            ...
            if (!prompt)
                throw new Error('Context-aware prompt not generated');
            if (prompt.length === 0)
        ...
        runTest('Backward Compatibility', () => {
            ...
            if (!legacyDefault)
                throw new Error('Legacy default prompt failed');
            if (!legacyAgentZero)
                throw new Error('Legacy agentZero prompt failed');
            if (!legacyCodeAssistant)
                throw new Error('Legacy codeAssistant prompt failed');
        ...
        runTest('Custom Instructions Integration', () => {
            ...
            if (!prompt)
                throw new Error('Prompt with custom instructions not generated');
        ...
        runTest('Configuration Validation', () => {
            ...
            if (!hasAgentInfo)
                throw new Error('Invalid agentInfo structure');
            if (!hasPrompts)
                throw new Error('Invalid prompts structure');
            if (!hasTemplateVars)
                throw new Error('Invalid templateVariables structure');
        ...
        runTest('Complex Template Variable Substitution', () => {
            const complexTemplate = llmHelper.getAgentPrompt('TestAgent', 'system.complex', { var1: 'value1', var2: 'value2', var3: 'value3' });
            if (!complexTemplate) {
...
//# sourceMappingURL=test-agent-prompts.js.map