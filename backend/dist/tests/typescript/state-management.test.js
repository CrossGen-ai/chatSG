...
exports.StateManagementTestUtils = void 0;
exports.testStateManagement = testStateManagement;
const StateManager_1 = require("../../src/state/StateManager");
const MemoryPersistence_1 = require("../../src/state/persistence/MemoryPersistence");
const FilePersistence_1 = require("../../src/state/persistence/FilePersistence");
...
class StateManagementTestUtils {
    static createTestSessionState(sessionId, overrides = {}) {
        return {
            sessionId,
            createdAt: new Date(),
    ...
    static createTestAgentState(agentName, overrides = {}) {
        return {
            agentName,
            isActive: false,
            lastUsed: new Date(),
    ...
    static async createTestStateManager(persistence) {
        ...
        if (persistence) {
            stateManager.persistence = persistence;
        }
        return stateManager;
    }
    static createTestFilePath(filename) {
    ...
    static async cleanupTestFiles(pattern) {
        const testDir = path.join(__dirname, 'test-data');
        if (fs.existsSync(testDir)) {
    ...
    static verifySessionState(state, expected) {
    ...
    static verifyAgentState(state, expected) {
        for (const [key, value] of Object.entries(expected)) {
            const actualValue = state[key];
...
exports.StateManagementTestUtils = StateManagementTestUtils;
async function testStateManagement() {
    ...
    incrementTest(runTest('StateManager Singleton Pattern', () => {
        const manager1 = StateManager_1.StateManager.getInstance();
        const manager2 = StateManager_1.StateManager.getInstance();
    ...
    incrementTest(await runAsyncTest('Memory Persistence Layer', async () => {
        const memoryPersistence = new MemoryPersistence_1.MemoryPersistence();
    ...
    incrementTest(await runAsyncTest('File Persistence Layer', async () => {
        ...
        const filePersistence = new FilePersistence_1.FilePersistence(testFilePath);
...
//# sourceMappingURL=state-management.test.js.map