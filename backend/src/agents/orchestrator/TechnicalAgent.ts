 ...
 */
export class TechnicalAgent extends SpecializedLLMAgent {
     ...
     */
    private identifyTechnicalTask(input: string): string | null {
    ...
    private detectProgrammingLanguage(input: string): string | null {
        ...
        if (lowerInput.includes('vue')) return 'Vue.js';
        if (lowerInput.includes('angular')) return 'Angular';
        if (lowerInput.includes('node')) return 'Node.js';
        if (lowerInput.includes('express')) return 'Express.js';
     ...
     */
    private identifyTechnicalDomain(input: string): string {
        const lowerInput = input.toLowerCase();
