...
class IntegrationTestAgent extends BaseAgent {
    constructor(
        private name: string,
        private agentType: string = 'integration',
    ...
    async processMessage(message: string, context?: any): Promise<any> {
        ...
        if (selectedTool) {
            try {
                ...
                console.warn(`Tool execution failed: ${(error as Error).message}`);
            }
        }
...
class IntegrationTestTool implements Tool {
    public name: string;
    public version: string = '1.0.0';
    public description: string;
    public category: string;
    public tags: string[] = ['integration', 'test'];
    public author: string = 'Integration Test Suite';

    constructor(name: string, category: string = 'general') {
        this.name = name;
...
export class IntegrationTestUtils {
    static async setupFullSystem(): Promise<{
    ...
    static async setupTestAgentsAndTools(
        agentRegistry: AgentRegistry,
        toolRegistry: ToolRegistry
    ): Promise<void> {
        // Register test tools
    ...
    static createTestWorkflow(): Array<{
        message: string;
        sessionId: string;
    ...
    static async measureSystemPerformance(
...
async function testIntegration(): Promise<void> {
    ...
    incrementTest(await runAsyncTest('Full System Initialization', async () => {
        const { orchestrator, agentRegistry, toolRegistry, stateManager } = 
            await IntegrationTestUtils.setupFullSystem();
    ...
    incrementTest(await runAsyncTest('Agent-Tool Integration', async () => {
        ...
        const agent = agentRegistry.getAgent('AnalyticalAgent') as IntegrationTestAgent;
        if (!agent) {
    ...
    incrementTest(await runAsyncTest('Concurrent Multi-Agent Processing', async () => {
        const { orchestrator, agentRegistry, toolRegistry } = 
            await IntegrationTestUtils.setupFullSystem();
        await IntegrationTestUtils.setupTestAgentsAndTools(agentRegistry, toolRegistry);
        ...
        const responses = await Promise.all(
            requests.map(request => orchestrator.processRequest(request))
        );
    ...
    incrementTest(await runAsyncTest('System Performance and Reliability', async () => {
        ...
        const performanceResults = await IntegrationTestUtils.measureSystemPerformance(
            testOperation,
            10
        );
...
export { testIntegration, IntegrationTestUtils }; 