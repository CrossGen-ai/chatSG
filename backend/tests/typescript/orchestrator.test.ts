...
class MockOrchestratorAgent extends BaseAgent {
    constructor(
        private name: string,
        private agentType: string = 'mock',
        private capabilities: AgentCapabilities = { processMessage: true, supportsSessions: false },
...
class MockBackendIntegration extends BackendIntegration {
    private responses: Map<string, any> = new Map();
    private requestLog: Array<{ endpoint: string; data: any; timestamp: Date }> = [];
    ...
    getRequestLog(): Array<{ endpoint: string; data: any; timestamp: Date }> {
...
export class OrchestratorTestUtils {
    static createMockAgent(
        ...
        return new MockOrchestratorAgent(
            name,
            options.type,
            capabilities,
    ...
    static async createTestOrchestrator(): Promise<AgentOrchestrator> {
        const orchestrator = AgentOrchestrator.getInstance();
        await orchestrator.initialize();
        return orchestrator;
    }

    static createMockBackend(): MockBackendIntegration {
        return new MockBackendIntegration();
    }

    static verifyOrchestratorResponse(
        response: any,
    ...
    static async measureResponseTime(asyncFunction: () => Promise<any>): Promise<number> {
        const start = Date.now();
        await asyncFunction();
        return Date.now() - start;
    }

    static createTestRequest(message: string, sessionId?: string, agentType?: string): any {
        return {
            message,
...
async function testOrchestrator(): Promise<void> {
    ...
    incrementTest(runTest('AgentOrchestrator Singleton Pattern', () => {
        const orchestrator1 = AgentOrchestrator.getInstance();
        const orchestrator2 = AgentOrchestrator.getInstance();
    ...
    incrementTest(await runAsyncTest('Orchestration Middleware', async () => {
        const orchestrator = await OrchestratorTestUtils.createTestOrchestrator();
        const middleware = new OrchestrationMiddleware();
    ...
    incrementTest(await runAsyncTest('Error Handling and Recovery', async () => {
        ...
        try {
            ...
            console.log(`   ⚠️  Exception: ${(error as Error).message}`);
            return true;
        }
    }));
    ...
    incrementTest(await runAsyncTest('Performance and Concurrent Processing', async () => {
        ...
        const requests = agents.map((agent, index) => 
            OrchestratorTestUtils.createTestRequest(
        ...
        const responses = await Promise.all(
            requests.map((request, index) => 
                orchestrator.processRequest(request, { targetAgent: agents[index].name })
            )
        );
...
export { testOrchestrator, OrchestratorTestUtils }; 