...
class SimpleTestUtils {
    static createTestData(name: string): { name: string; timestamp: Date; id: number } {
        return {
            name,
            timestamp: new Date(),
    ...
    static verifyObject(obj: any, expectedProperties: string[]): void {
...
async function testSimpleTypeScript(): Promise<void> {
    ...
    incrementTest(runTest('Basic TypeScript Compilation', () => {
        ...
        const config: { enabled: boolean; timeout: number } = {
            enabled: true,
            timeout: 5000
        };
    ...
    incrementTest(runTest('Error Handling', () => {
        try {
            ...
            if ((error as Error).message.includes('missing property: missing')) {
                console.log('   ✅ Error handling working correctly');
                console.log(`   ⚠️  Caught expected error: ${(error as Error).message}`);
                return true;
            } else {
                throw error;
            }
...
export { testSimpleTypeScript, SimpleTestUtils }; 