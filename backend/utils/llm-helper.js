...
class LLMHelper {
    ...
    getTemperature(environment) {
        ...
        if (!isNaN(envTemp)) return envTemp;
    ...
    getMaxTokens(environment) {
        ...
        if (!isNaN(envTokens)) return envTokens;
    ...
    convertParametersToJSONSchema(parameters) {
        ...
        for (const param of parameters) {
            ...
            if (param.format) properties[param.name].format = param.format;
    ...
    createLLMWithStructuredOutput(schema, options = {}) {
        const {
            method = this.config.supportsStructuredOutput ? 'json_schema' : 'json_mode',
            includeRaw = false,
            strict = this.config.supportsStructuredOutput,
            ...llmOptions
        } = options;
